...
void RakibOne8()
{
	...
	auto dfs = [&](auto && self, int node, int parent, int depth)->int{
		int answer = depth * a[node];
		subtreeSum[node] = a[node];

		for (auto child : adj[node]) {
			if (child != parent) {
				answer += self(self, child, node, depth + 1);
	...
	auto reroot = [&](auto && self, int root, int parent, int ans)->void{
		...
		for (auto child : adj[root]) {
			if (child != parent) {
				int nextAns = (dp[root] + subtreeSum[1] - 2 * subtreeSum[child]);
				self(self, child, root, nextAns);
			}
		}
	};
	reroot(reroot, 1, -1, dfs(dfs, 1, -1, 0));
	debug(dp, subtreeSum);
