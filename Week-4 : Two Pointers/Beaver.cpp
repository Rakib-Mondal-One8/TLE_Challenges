...
void solve()
{
    ...
    for (right = 0;right < s.size();right++) {
        for (auto check : a) {
            if (right - left + 1 >= check.size() && s.substr(right - check.size() + 1, check.size()) == check) {
                left = right - check.size() + 2;
            }
        }
        if (ans.empty() || ans[0] < right - left + 1) {
            ans = vector<int>{ right - left + 1,left };
        }
    }
    cout << ans[0] << " " << ans[1] << nl;
}
