...
void solve()
{
	...
	while (l <= r) {
		...
		array<int, 3>res;
		if (mid - 1 <= 0)res[0] = INT_MAX;
		else res[0] = ask(mid - 1);

		res[1] = ask(mid);

		if (mid + 1 > n)res[2] = INT_MAX;
		else res[2] = ask(mid + 1);

		if (res[0] > res[1] && res[1] < res[2]) {
			ans = mid;
			break;
		}
		else if (res[0] < res[1] && res[1] < res[2])r = mid - 1;
		else if (res[0] > res[1] && res[1] > res[2])l = mid + 1;
		else r = mid - 1;
	}
	cout << "! " << ans << endl;
	cout.flush();
