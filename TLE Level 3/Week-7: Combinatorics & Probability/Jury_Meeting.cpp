...
void solve()
{
	...
	int secondMx = -1;
	for (auto x : a) {
		if (x != mx)secondMx = max(x, secondMx);
	}
	...
	else if (abs(mx - secondMx) > 1) {
		cout << 0 << nl;
	}
	else
	{
		Mint ans = fac(n);
		int otherValues = (n - (cnt[mx] + cnt[secondMx]));
		for (int i = cnt[secondMx]; i < n; i++) {
			Mint c = fac(i) * ncr(otherValues, i - cnt[secondMx]) * fac(n - (i + 1));
			ans -= c;
		}
		cout << ans.val << nl;
	}
