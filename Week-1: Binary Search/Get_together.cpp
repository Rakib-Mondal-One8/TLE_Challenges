...
bool check(vector<double>&a, vector<double>&b, int n, double x) {
	pair<double, double> cur = {a[0] - (b[0]*x), a[0] + (b[0]*x)};
	loop(i, 1, n - 1) {
		pair<double, double> now = {a[i] - (b[i]*x), a[i] + (b[i]*x)};
		pair<double, double> res = inter(cur, now);
		if (res.first <= res.second)cur = res;
		else return false;
	}
	return true;
}
void solve()
{
	...
	double l = 0, r = 1e18, precision = 1e-7;
	double ans;
	for (int i = 0; i < 200; i++) {
		double mid = l + (r - l) / 2;
		if (check(a, v, n, mid)) {
