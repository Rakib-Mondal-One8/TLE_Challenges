...
void solve()
{
    ...
    set<ll>currentDivisors;
    vector<ll>newDivisors;
    int ans = 1;
    for (int i = 0;i < n;i++) {
        for (auto val : currentDivisors) {
            if (val % a[i] == 0)newDivisors.push_back(val / a[i]);
        }
        bool flag = false;
        for (auto val : newDivisors) {
            if (val == 1)flag = true;
            currentDivisors.insert(val);
        }
        newDivisors.clear();
        if (flag) {
            ans++;
        ...
        if (x % a[i] == 0)currentDivisors.insert(x / a[i]);
    }
    cout << ans << nl;
}
int32_t main()
