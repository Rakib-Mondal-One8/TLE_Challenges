...
void RakibOne8()
{
	...
	int l = -1, r = -1;
	for (int i = 0; i < n; i++) {
		vector<int>pref = hs.substringHash(0, i);
		vector<int>suff = hs.substringHash(n - i - 1, n - 1);

		if (pref == suff) {
			debug(pref, suff);
			int len = i + 1;
			for (int j = 1; j + len - 1 < n - 1 ; j ++) {
				if (pref == hs.substringHash(j, j + len - 1)) {
					l = 0;
					r = i;
					break;
				}
	...
	if (r == -1)cout << "Just a legend" << nl;
	else cout << s.substr(l, r + 1) << nl;
}
int32_t main()
{
	init_code();
